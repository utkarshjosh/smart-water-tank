...
import { Broadcast, CheckCircle, Cpu, DownloadSimple, FileArrowUp, HardDrive, Package, ArrowCounterClockwise, Trash, UploadSimple, WarningCircle } from '@phosphor-icons/react';
...
export default function FirmwarePage() {
    ...
    <AppShell variant="admin">
      <div className="space-y-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-slate-950 ">Firmware management</h1>
 ...
 return (
        ...
        <Card className="border-slate-200 shadow-sm ">
          <CardHeader className="border-b border-slate-100 p-4 ">
            <CardTitle className="text-base font-semibold tracking-normal">UploadSimple firmware</CardTitle>
 ...
 required
                  ...
                  <div className="flex flex-col items-center justify-center space-y-2">
                    <FileArrowUp className={`h-8 w-8 ${isDragging || selectedFile ? 'text-sky-600 ' : 'text-slate-400'}`} />
          ...
          <CardContent className="p-0">
            {firmware.length === 0 ? (
              <div className="p-8 text-center">
                ...
                <p className="text-xs text-slate-500 ">UploadSimple a .bin package to begin rollout management.</p>
              </div>
            ) : (
                  ...
                  <div key={fw.id} className="grid gap-3 p-4 lg:grid-cols-[1fr_140px_310px] lg:items-center">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                          <p className="text-sm font-semibold text-slate-950 ">Version {fw.version}</p>
                          {fw.is_active && (
                            <Badge variant="brand">Active</Badge>
                          )}
                      </div>
                        {fw.description && (
                        <p className="mt-1 line-clamp-2 text-sm text-slate-600 ">{fw.description}</p>
                        )}
                    ...
                    <div>
                      <div className="flex items-center justify-between text-xs text-slate-500 ">
                        <span>Rollout</span>
                        <span className="font-medium text-slate-700 ">{fw.rollout_percentage}%</span>
                      </div>
