...
export default function FirmwarePage() {
  ...
  const handleUpload = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(errorMessage(err, 'Failed to upload firmware'));
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleDownload = async (fw: Firmware) => {
    ...
    try {
      ...
      setError(errorMessage(err, 'Failed to download firmware'));
    } finally {
  ...
  const handleFirmwareAction = async () => {
    ...
    try {
      ...
      setFirmwareActionError(errorMessage(err, `Failed to ${type} firmware`));
    } finally {
  ...
  const handleRollout = async () => {
    ...
    try {
      ...
      setError(errorMessage(err, 'Failed to rollout firmware'));
    } finally {
      setRollingOut(false);
    }
  };
  ...
  if (loading) {
    // Shaped like the page below rather than a centred spinner, so the layout
    // does not jump when the three requests land.
    return (
      <div className="space-y-5" aria-busy="true">
        <span className="sr-only">Loading firmware</span>
  ...
  const firmwareStats = [
    ...
    },
    {
      ...
      detail: `${tenants.length} tenant${tenants.length === 1 ? '' : 's'}`,
    },
    {
      label: 'Storage',
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Firmware"
        description="Upload binaries, track deployed builds and stage rollouts."
        actions={
          activeFirmware && (
            <Badge variant="good">
              <CheckCircle size={13} weight="fill" aria-hidden />v{activeFirmware.version} active
            </Badge>
          )
        }
      />
      ...
      <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
        {firmwareStats.map((stat) => (
          <StatTile key={stat.label} label={stat.label} value={stat.value} hint={stat.detail} />
        ))}
      </div>
      ...
      <div className="grid gap-3 xl:grid-cols-[0.82fr_1.18fr]">
        <Card className="border-hairline">
          <CardHeader className="border-b border-hairline p-4">
            <CardTitle className="text-title tracking-normal">Upload firmware</CardTitle>
          ...
          <CardContent className="p-0">
            {firmware.length === 0 ? (
              <div className="p-8 text-center">
                ...
                <p className="text-caption text-ink-3">
                  Upload a .bin package to begin rollout management.
                </p>
              </div>
            ) : (
              <div className="divide-y divide-hairline">
                {firmware.map((fw) => (
                  <div
                    key={fw.id}
                    // Stacked rather than three columns: this card sits in the
                    // narrow half of a two-column page, where a `lg:` grid
                    // (a viewport query, not a container one) crushed the
                    // description into a 60px ribbon of wrapped words.
                  ...
                  >
                    ...
                    {fw.description && (
                      <p className="line-clamp-2 text-body text-ink-2">{fw.description}</p>
                    )}
                    ...
                    <div>
                      ...
                      <div
                        ...
                        role="meter"
                        aria-label={`Rollout of ${fw.version}`}
                        aria-valuenow={fw.rollout_percentage}
                        aria-valuemin={0}
                        aria-valuemax={100}
                      >
                        <div
                          className="h-full rounded-full bg-brand transition-[width] duration-smooth ease-emphasized"
                        ...
                        <div className="space-y-3">
                          {devices.map((device) => (
                            ...
                            >
                              ...
                              >
                                <div className="text-label">{device.name || device.device_id}</div>
