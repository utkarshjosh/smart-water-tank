    ...
    <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
            ...
            {STEPS.map((item, index) => (
              <span
                ...
                className={`rounded px-2 py-1 text-center text-xs font-semibold ${
                  index === activeStepIndex ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-500'
                }`}
              >
                {item.label}
              </span>
      ...
      <div className="grid gap-0 md:grid-cols-[0.9fr_1.1fr]">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="rounded-md bg-surface-sunk px-3 py-2 ring-1 ring-hairline">
                ...
                <p className="mt-1 truncate text-sm font-semibold">
                  {totalCapacityL > 0 ? `${totalCapacityL.toFixed(0)}L` : 'Pending'}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-4 p-4">
          {error && (
            <div className="w-full rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-red-800">
              <div className="flex items-start gap-3">
          ...
          {step === 'shape' && (
            <div className="space-y-3">
              <div>
                <p className="text-sm font-semibold text-slate-950">What shape is your tank?</p>
                <p className="text-sm text-slate-500">Choose the closest physical layout.</p>
              </div>
          ...
          {step === 'parallel' && (
            <div className="space-y-3">
              <div>
                <p className="text-sm font-semibold text-slate-950">Connected tank units</p>
                <p className="text-sm text-slate-500">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Button
                  type="button"
                  variant={form.parallelUnitCount === 1 ? 'primary' : 'secondary'}
                ...
                <Button
                  ...
                  onClick={() =>
                    setForm((f) => ({
                      ...f,
                      parallelUnitCount: f.parallelUnitCount > 1 ? f.parallelUnitCount : 2,
                    }))
                  }
                >
                  Yes, multiple
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        parallelUnitCount: Math.min(6, Math.max(2, Number(e.target.value) || 2)),
                      }))
                    }
                  />
                </div>
              ...
              <div className="flex gap-3">
                <Button variant="secondary" className="w-full" onClick={() => setStep('shape')}>
                  Back
                </Button>
          ...
          {step === 'dimensions' && (
            <div className="space-y-3">
              ...
              <div>
                ...
                {showAdvanced && (
                  <div className="mt-2 space-y-4 rounded-lg border border-slate-200 p-3">
                    <div className="space-y-2">
                      <Label htmlFor="sensor-offset-cm">Sensor mounting offset (cm)</Label>
                      <p className="text-xs text-muted-foreground">
                        How far below the tank's rim is the sensor mounted? Leave as 0 unless your
                        sensor sits noticeably below the rim.
                      </p>
                      <Input
                        id="sensor-offset-cm"
                        type="number"
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Minimum distance the ultrasonic sensor can measure. Water closer than this
                        reads as full. Most SR04-style sensors are ~20-25cm.
                      </p>
                      <Input
                        id="dead-zone-cm"
                        type="number"
              ...
              <div className="flex gap-3">
                <Button variant="secondary" className="w-full" onClick={() => setStep('parallel')}>
                  Back
                </Button>
                <Button
                  className="w-full"
                  disabled={!canProceedFromDimensions}
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-3">
                ...
                <Button className="w-full" onClick={save} disabled={saving}>
                  {!saving && <FloppyDisk className="mr-2 h-4 w-4" />}
