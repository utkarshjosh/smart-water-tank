...
export default function MeasurementExportDialog({ devices, endpoint }: MeasurementExportDialogProps) {
 ...
 Export data
      ...
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
 DownloadSimple a CSV for the selected devices and time range. Times in the file are UTC; the fields below use your local time.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex gap-3 text-xs">
                <button type="button" className="font-medium text-sky-700 hover:underline " onClick={() => setSelectedIds(devices.map((device) => device.id))}>
 Select all
                </button>
                <button type="button" className="font-medium text-slate-500 hover:underline " onClick={() => setSelectedIds([])}>
 Clear
                </button>
              </div>
            </div>
 ...
 return (
                  <label key={device.id} className="flex cursor-pointer items-center gap-3 px-3 py-2.5 hover:bg-slate-50 ">
 ...
 Exports are limited to 366 days, 100 devices, and 100,000 rows. Narrow the range if the row limit is exceeded.
        ...
        <DialogFooter>
          <Button type="button" variant="secondary" onClick={() => setOpen(false)} disabled={exporting}>Cancel</Button>
