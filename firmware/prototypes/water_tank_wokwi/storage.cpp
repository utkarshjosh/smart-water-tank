/**
 * Storage Module Implementation (Wokwi / ESP32 build)
 * Same buffering logic as firmware/src/modules/storage.cpp - only the
 * directory-listing calls are ported from ESP8266's Dir/FSInfo API to
 * ESP32 LittleFS's File::openNextFile()/totalBytes()/usedBytes() API.
 */
...
namespace Storage {
    void init() {
        ...
        if (!LittleFS.begin(true)) {
            Serial.println(F("[Storage] Mount/format failed!"));
            return;
        }
    ...
    void bufferMeasurement(const SystemState& state) {
        if (bufferCounter >= MAX_BUFFER_FILES) {
            ...
            if (oldest) {
                String oldestPath = String(oldest.path());
                oldest.close();
